...
function rssmi_video($link, $targetWindow)
{  //  CHECKS IF VIDEO COMES FROM YOUTUBE OR VIMEO
    if (strpos($link, 'www.youtube.com') > 0) {
        if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $link, $match)) {
            $videoId = $match[1];
            $vLink = 'http://www.youtube.com/embed/' . $videoId . '?rel=0&amp;wmode=transparent';
        ...
        if (preg_match_all('#vimeo\.com/(\w*/)*(\d+)#i', $link, $match)) {
            $videoId = $match[2][0];
            $vLink = 'http://player.vimeo.com/video/' . $videoId;
    ...
    $vLink = (!empty($vLink) ? $vLink : null);
    $openWindow = (!empty($openWindow) ? $openWindow : null);
    $videoId = (!empty($videoId) ? $videoId : null);
    return array($vLink, $openWindow, $videoId);
}
...
{  ///this fixes the ever present facebook image problem
...
function rssmi_facebook_autopost($mediaImage)
{  ///this fixes the ever present facebook image problem
...
function rssmi_resize_image($imgHtml)
{
    ...
    return (isset($returnImage) ? $returnImage : false);
}
